"""
Video Language Identification Pipeline using ECAPA-TDNN Model
Extracts audio from video using ffmpeg (WAV format), identifies language
using speechbrain/lang-id-voxlingua107-ecapa model, transcribes with Whisper, 
and translates to English.

Whisper language codes reference: https://whisper-api.com/docs/languages/
"""
...
# Whisper supported language codes (from https://whisper-api.com/docs/languages/)
# ECAPA-TDNN returns ISO 639-1 codes which are compatible with Whisper
...
def validate_whisper_language(language_code: str) -> Optional[str]:
    """
    Validates and normalizes language code for Whisper.
    ECAPA-TDNN returns ISO 639-1 codes which are compatible with Whisper.
    
    Args:
        language_code: Language code from ECAPA model
    
    Returns:
        Validated Whisper language code or None if not supported
    """
    # Normalize to lowercase
    lang_lower = language_code.lower().strip()
    
    # Check if it's in Whisper's supported languages
    if lang_lower in WHISPER_SUPPORTED_LANGUAGES:
        return lang_lower
    
    # Return None to let Whisper auto-detect
    print(f"Warning: Language code '{language_code}' may not be supported by Whisper, will auto-detect")
    return None
...
def process_video_pipeline(video_path: str, output_json_path: Optional[str] = None, keep_audio: bool = False, output_wav_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Step 4: Validate language code for Whisper
        # ECAPA returns ISO 639-1 codes which are compatible with Whisper
        whisper_lang = validate_whisper_language(language_code)
        if whisper_lang:
            print(f"Using Whisper language code: {whisper_lang}")
        else:
            print(f"Language code '{language_code}' not in Whisper supported list, will auto-detect")
        ...
        output_data = {
            ...
            "language_detection": {
                ...
                "whisper_language_code": whisper_lang or language_code,
                "confidence": confidence
            },
            "transcription": {
                "text": transcription_result.get('text', ''),
                "language": transcription_result.get('language', whisper_lang or language_code or 'auto'),
                "segments": [
                    {
        ...
        print(f"Whisper Language: {transcription_result.get('language', 'auto')}")
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("Usage: python video_lid_ecapa_pipeline.py <video_file_path> [output_json_path] [output_wav_path]")
        print("\nExample:")
        print("  python video_lid_ecapa_pipeline.py /path/to/video.mp4")
        print("  python video_lid_ecapa_pipeline.py /path/to/video.mp4 /path/to/output.json")
        print("  python video_lid_ecapa_pipeline.py /path/to/video.mp4 /path/to/output.json /path/to/output.wav")
        print("\nModel: speechbrain/lang-id-voxlingua107-ecapa (supports 107 languages)")
        print("Pipeline: LID -> Whisper Transcription -> English Translation -> JSON Output")
        print("Whisper language codes: https://whisper-api.com/docs/languages/")
        sys.exit(1)
